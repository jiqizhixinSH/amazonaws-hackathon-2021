...
def ACNN(params):
    ...
    if(params.model == 'ACNN'):
    
        input_seq = Input((96,112,3))
        
        
        #trunk branch
        BN2 = BatchNormalization( momentum= 0.9, epsilon= 1e-04)(input_seq)
        conv2   = Conv2D(256, 3, strides=1, padding="same")(BN2)
        act1 = Activation('relu')(conv2)
        conv3   = Conv2D(256, 1, strides=1, padding="same")(act1)
        act2 = Activation('relu')(conv3)
        conv4   = Conv2D(256, 3, strides=1, padding="same")(act2)
        act3 = Activation('relu')(conv4)
        conv5   = Conv2D(256, 1, strides=1, padding="same")(act3)
